...
SCENARIO("RB delete non inserted element", "[delete]")
{
	RBtree<int, int> tree = { {8, 7} };
	tree.Remove(4);
	REQUIRE(!tree.isEmpty());
}
SCENARIO("RB delete root with one element", "[delete]")
{
	RBtree<int, int> tree = { {8,8} };
	tree.Remove(8);
	REQUIRE(tree.isEmpty());
}
SCENARIO("RB delete root without child", "[delete]")
{
...
SCENARIO("RB delete root with two children", "[delete]")
{
...
SCENARIO("RB delete non root without children", "[delete]")
{
	RBtree<int, int> tree = { {1, 8}, {18, 4}, {22, 3}, {2,10}, {3, 9}, {12,13}, {7,11}, {1,12} };
	tree.Remove(3);
...
SCENARIO("RB delete non root with one child", "[delete]")
{
	RBtree<int, int> tree = { {1, 8}, {18, 4}, {22, 3}, {2,10}, {3, 9}, {12,13}, {7,11}, {1,12} };
	tree.Remove(12);
